...
gen_t_data <- function(n_genes, n_experiments, prop_affec) {
  ...
  gen_fit_mixture <- function() {
    ...
    for(i in 1:n_genes) {
      return_vec[i] <- ifelse(assign[i] == 1, runif(1,.15, 1), 1)
    }
    return(return_vec)
  }
  
  t0 <- rpois(n_genes , n_experiments)
  ...
  experiment_object <- Map(gen_mixture, mixing = rbeta(n_genes, 5,5), n = result, n0 = t0, num = n_experiments)
